...
import { UsersIcon, UserGroupIcon, CalendarIcon, BellIcon } from '@/components/icons';
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <Link href="/mentor/profile" className="bg-white border border-gray-200 p-4 hover:border-primary-green transition-colors">
            <div className="text-2xl font-bold text-primary-green">{mentor?.reviewCount || 0}</div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {request.message && (
                          <p className="text-sm text-muted-text mt-1 line-clamp-1">{request.message}</p>
                        )}
                      </div>
                    </div>
        ...
        <div className="bg-white border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href="/mentor/mentees" className="text-sm text-primary-green hover:underline">
              View all →
            </Link>
          </div>
                ...
                <div key={mentorship.id} className="border border-gray-100 p-3">
                  <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-full bg-primary-green/10 flex items-center justify-center text-xs font-semibold text-primary-green">
                      {mentorship.student?.fullName?.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) || '?'}
                    </div>
                    <div>
                      <h4 className="font-semibold">{mentorship.student?.fullName}</h4>
                      <p className="text-xs text-muted-text">{mentorship.student?.programme}</p>
                    </div>
                  </div>
                </div>
              ))}
...
function QuickAction({ href, label, icon: Icon }: { href: string; label: string; icon: any }) {
  return (
    <Link href={href} className="bg-white border border-gray-200 p-4 text-center hover:border-primary-green transition-colors group">
