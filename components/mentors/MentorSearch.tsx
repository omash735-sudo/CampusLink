// components/mentors/MentorSearch.tsx
'use client';
...
export function MentorSearch() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (query.trim()) {
      router.push(`/mentors?search=${encodeURIComponent(query.trim())}`);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="relative">
      <input
        type="text"
        placeholder="Search by name, expertise, programme, career field..."
        value={query}
