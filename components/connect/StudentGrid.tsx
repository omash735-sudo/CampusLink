...
function StudentCard({ student }: { student: StudentCardProps }) {
  ...
  return (
    <Link
      ...
      className="border border-gray-200 bg-white hover:border-primary-green transition-colors p-4"
    >
      <div className="flex items-start gap-3">
        <div className="h-12 w-12 flex-shrink-0 border border-gray-200 bg-primary-green text-white flex items-center justify-center font-semibold text-lg">
          {student.avatar ? (
        ...
        <div className="flex-1 min-w-0">
          ...
          {student.year && (
            <p className="text-sm text-muted-text flex items-center gap-1">
              <AcademicIcon className="h-3 w-3" />
              Year {student.year}
            </p>
          )}
