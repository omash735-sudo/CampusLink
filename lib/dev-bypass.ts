...
async function resolveBypassUser(): Promise<AuthUser> {
  ...
  try {
    ...
    // fall through to anonymous bypass user
  }
...
/**
 * Admin/publications gate.
 *
 * - With DEV_BYPASS=true: returns a bypass admin without checking session.
 * - Otherwise: real auth via getCurrentUser() + role check.
 *
 * The bypass only skips the authorization step. All downstream code
 * (database writes, storage, audit) runs identically.
 */
export async function requireAdminOrPublications(): Promise<AuthUser | null> {
  if (DEV_BYPASS_ENABLED) {
    const u = await resolveBypassUser();
    console.warn(
      `[DEV_BYPASS] admin authorization bypassed — writing as ${u.email}`
    );
    return u;
  }
